...
import { UpcomingIndustryEvents } from "@/components/UpcomingIndustryEvents";
...
const facadePracticeArticles = [
  ...
  {
    path: "/articles/metall-dlya-goroda-proekty-stal-produkt",
    title: "Металл для города: где работают изделия «Сталь Продукт»",
    lead: "Жилые кварталы, медицинские и образовательные объекты: показываем реальные поставки и объясняем, как металлокассеты, решётки, кронштейны, корпуса и другие изделия объединяются в одном строительном проекте.",
        ...
        <section className="journal-hero relative overflow-hidden border-b border-white/12 pt-[76px]">
          ...
          <div className="container relative z-10 grid min-h-[440px] items-end gap-10 py-16 lg:grid-cols-[minmax(0,1fr)_330px] lg:py-20">
            <div>
              ...
              <h1 className="mt-5 max-w-4xl text-4xl font-semibold uppercase leading-[1.02] sm:text-5xl lg:text-6xl">Инженерный журнал<span className="mt-2 block text-steel-orange">Сталь Продукт</span></h1>
              <p className="mt-6 max-w-2xl text-base leading-8 text-white/68">Разбираем реальные производственные задачи, подготовку документации, выбор материалов, технологии обработки листового металла и фасадные решения.</p>
            </div>
            <div className="border-l-2 border-steel-orange bg-black/35 p-6 backdrop-blur-sm">
              ...
              <p className="mt-3 text-sm leading-7 text-white/67">Опираемся на инженерную логику, производственный опыт и первоисточники. Каждый материал должен помогать принять практическое решение.</p>
            </div>
          </div>
        </section>
          ...
          <div className="container grid items-start gap-8 lg:grid-cols-[260px_minmax(0,1fr)] xl:gap-12">
            <aside className="journal-sidebar lg:sticky lg:top-6">
              <div className="border border-white/12 bg-[#0f1418]">
                <div className="border-b border-white/10 px-5 py-4"><p className="text-xs font-bold uppercase tracking-[.16em] text-white/42">Навигация по журналу</p></div>
                <nav aria-label="Разделы инженерного журнала">
                  {navigation.map((item) => (
                    <a key={item.id} href={`#${item.id}`} className="journal-sidebar-link group grid grid-cols-[52px_minmax(0,1fr)] items-center border-b border-white/8 px-5 py-4 last:border-0">
                      <span className="text-lg font-bold tabular-nums text-steel-orange">{item.marker}</span>
              ...
              <div className="mt-4 border border-steel-orange/35 bg-[linear-gradient(135deg,rgba(234,91,12,.14),rgba(15,20,24,.98)_54%)] p-5">
                ...
                <Link href="/contacts#contact-form" className="mt-4 inline-flex text-xs font-bold uppercase tracking-[.06em] text-steel-orange">Написать нам&nbsp; →</Link>
              </div>
            </aside>
            ...
            <div className="min-w-0">
                  ...
                  <div className="mt-7 grid gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(300px,.65fr)]">
                    <article className="journal-card-feature group relative min-h-[510px] overflow-hidden border border-steel-orange/45 bg-[#101519]">
                      ...
                      <div className="absolute inset-x-0 bottom-0 p-6 sm:p-8">
                        ...
                        <Link href={`/articles/${featuredEngineeringArticle.slug}`} className="mt-6 inline-flex border border-steel-orange/65 px-4 py-3 text-xs font-bold uppercase tracking-[.06em] text-steel-orange transition hover:bg-steel-orange-deep hover:text-white">Читать материал&nbsp; →</Link>
                      </div>
                    </article>
                    <div className="grid gap-4">{secondaryEngineeringArticles.map((article) => <ArticleCard key={article.slug} article={article} compact />)}</div>
                  </div>
                ) : null}
                {remainingEngineeringArticles.length ? <div className="mt-4 grid gap-4 xl:grid-cols-2">{remainingEngineeringArticles.map((article) => <ArticleCard key={article.slug} article={article} />)}</div> : null}
              </section>
                ...
                <div className="mt-7 grid gap-4 xl:grid-cols-2">
                  {facadePracticeArticles.map((article) => (
                    <Link key={article.path} href={article.path} className="group flex h-full flex-col border border-steel-orange/35 bg-[linear-gradient(145deg,rgba(224,86,36,.12),rgba(16,21,25,.98)_45%)] p-6 transition hover:border-steel-orange sm:p-7">
                      <div className="flex flex-wrap items-center gap-3 text-xs font-bold uppercase tracking-[.08em]"><span className="text-steel-orange">{article.label}</span><time dateTime={article.date} className="text-white/42">{formatDate(article.date)}</time></div>
              ...
              <section id="events" className="scroll-mt-24 border-t border-white/10 py-14 sm:py-16">
                <SectionHeading number="04" eyebrow="Проверенные календари" title="Выставки и события" description="Даты, города, направления оборудования и официальные сайты — в компактном формате для планирования поездок." />
                ...
                <div className="mt-7 grid gap-4 xl:grid-cols-2">
                  <Link href="/articles/vystavki-metalloobrabotka-kitay-2026" className="journal-event-card group border border-white/12 bg-[#101519] p-6 transition hover:border-steel-orange">
                    <span className="text-4xl font-bold tabular-nums text-steel-orange">14</span><p className="mt-5 text-xs font-bold uppercase tracking-[.12em] text-white/42">Китай · 2026–2027</p><h3 className="mt-3 text-xl font-semibold uppercase leading-tight">Выставки металлообработки</h3><p className="mt-4 text-sm leading-7 text-white/56">Резка, гибка, станки, автоматизация и промышленная робототехника.</p><span className="mt-6 inline-flex text-xs font-bold uppercase text-steel-orange">Открыть календарь&nbsp; →</span>
                  </Link>
                  <Link href="/articles/vystavki-fasady-arhitektura-2026" className="journal-event-card group border border-white/12 bg-[#101519] p-6 transition hover:border-steel-orange">
                    <span className="text-4xl font-bold tabular-nums text-steel-orange">16</span><p className="mt-5 text-xs font-bold uppercase tracking-[.12em] text-white/42">Россия · Китай · Дубай</p><h3 className="mt-3 text-xl font-semibold uppercase leading-tight">Фасады и архитектурные инновации</h3><p className="mt-4 text-sm leading-7 text-white/56">Фасадные системы, оболочка здания, стекло, панели, покрытия и проектирование.</p><span className="mt-6 inline-flex text-xs font-bold uppercase text-steel-orange">Открыть календарь&nbsp; →</span>
                  </Link>
                </div>
              </section>
            </div>
