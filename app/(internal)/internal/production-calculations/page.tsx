      ...
      {reports.length === 0 ? <p className="text-sm text-white/45">Производственных отчётов пока нет.</p> : <div className="overflow-x-auto">
        <table className="w-full min-w-[1080px] text-left text-sm">
          <thead className="border-b border-white/10 text-[10px] uppercase tracking-[.12em] text-white/35">
            <tr><th className="p-3">Дата</th><th className="p-3">Проект</th><th className="p-3">Позиции</th><th className="p-3">Подтверждено</th><th className="p-3">Готовность</th><th className="p-3">Статус</th><th className="p-3">База</th><th className="p-3"></th></tr>
          </thead>
          <tbody>{reports.map((report) => {
            const status = report.readinessStatus === "ready" ? "ready" : report.readinessStatus === "blocked" ? "critical" : "warning";
            const label = report.readinessStatus === "ready" ? "готов" : report.readinessStatus === "blocked" ? "заблокирован" : "требует проверки";
            return <tr key={report.fileName} className="border-b border-white/[.06] align-top">
              ...
              <td className="p-3"><div className="min-w-28"><div className="flex items-center justify-between text-xs"><span>{report.readinessScorePct}%</span><span className="text-white/35">{report.completeParts}/{report.totalParts}</span></div><div className="mt-2 h-1.5 overflow-hidden bg-white/10"><div className="h-full bg-steel-orange" style={{ width: `${report.readinessScorePct}%` }} /></div></div></td>
              <td className="p-3"><StatusPill status={status} label={label} /></td>
