      ...
      {report.calculation.parts.map((part, index) => {
        ...
        return <Panel key={part.partId} title={`Позиция ${index + 1} · ${part.partId}`}>
          ...
          {parameters && <div className="grid gap-3 text-sm sm:grid-cols-2 xl:grid-cols-4">
            ...
            <div className="border border-white/10 p-3"><div className="text-white/35">Отход</div><div className="mt-1">{number(parameters.stock.wasteAreaMm2, " мм²")} · {number(parameters.stock.wastePct, "%")}</div></div>
