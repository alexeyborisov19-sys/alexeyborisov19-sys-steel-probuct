...
export default async function NewSubjectRequestPage() {
  ...
  return <InternalShell {...shell}><InternalPageHeader eyebrow="Регистрация" title="Новое обращение субъекта" description="Указывайте только необходимый объём данных. Копия паспорта не требуется по умолчанию." /><Panel><SecureMutationForm csrfToken={shell.csrfToken} endpoint="/api/internal/personal-data/subject-requests" submitLabel="Зарегистрировать" successHref="/internal/personal-data/subject-requests/{id}" baseBody={{ responsibleUserId: userId }} fields={[
    ...
    { name: "confirmedDueAt", label: "Подтверждённый срок ответа", kind: "datetime-local", required: true, help: "Расчёт не учитывает официальные праздничные и перенесённые выходные дни. Проверьте срок по действующему законодательству и производственному календарю." },
    { name: "dueConfirmationBasis", label: "Основание подтверждения срока", kind: "textarea", required: true, help: "Укажите норму, документ или результат ручной проверки. Техническая расчётная дата сохраняется отдельно." }, { name: "legalBasis", label: "Правовое основание", kind: "textarea", required: true },
