...
const solutionCardImages: Record<string, { src: string; position?: string }> = {
  ...
  "Решения для кондиционирования": { src: "/images/industry/cards/climate.jpg", position: "center 48%" },
...
export default function Home() {
  return <><JsonLd data={webPageSchema({ name: "Сталь Продукт — инженерные решения из листового металла", description: "Проектирование, производство и поставка изделий из листового металла для строительства, промышленности и инженерной инфраструктуры.", path: "/" })} /><Header /><main><Hero />
    <section id="solutions" className="bg-[#0c1013] py-14"><div className="container"><h2 className="text-center text-2xl font-semibold uppercase tracking-wide sm:text-3xl">Решения для ваших задач</h2><div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{solutions.map((solution, index) => { const cardImage = solutionCardImages[solution.title]; return <Link key={solution.title} href={solution.href} className="group min-h-[250px] border border-white/15 bg-[#111519] p-5 transition hover:border-steel-orange hover:bg-[#15191c]"><div className={`relative -mx-5 -mt-5 mb-5 h-28 overflow-hidden bg-[#192026] ${solution.imageClassName ?? ""}`}><img src={cardImage.src} alt="" width={960} height={540} loading={index < 2 ? "eager" : "lazy"} fetchPriority={index === 0 ? "high" : "auto"} decoding="async" style={{ objectPosition: cardImage.position }} className="h-full w-full object-cover brightness-[1.06] contrast-[1.02] saturate-[1.04] transition duration-500 group-hover:scale-[1.035] group-hover:brightness-[1.14]" /></div><p className="text-lg text-steel-orange">{solution.icon}</p><h3 className="mt-3 text-sm font-bold uppercase leading-tight">{solution.shortTitle}</h3><p className="mt-3 text-[11px] leading-relaxed text-white/55">{solution.text}</p><span className="mt-5 block text-[10px] font-bold uppercase text-steel-orange">Перейти к решениям&nbsp; →</span></Link>})}</div></div></section>
