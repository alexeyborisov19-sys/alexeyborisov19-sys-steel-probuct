...
export default async function ArticlePage({ params }: ArticlePageProps) {
  ...
  return <><JsonLd data={[webPageSchema({ name: article.title, description: article.lead, path }), breadcrumbSchema([{ name: "Главная", path: "/" }, { name: "Статьи", path: "/articles" }, { name: article.title, path }]), articleSchema({ headline: article.title, description: article.lead, path, image: article.image, datePublished: article.publishedAt, dateModified: article.modifiedAt, citations: article.sources?.map((source) => source.url) })]} /><Header /><main>
    ...
    <article className="bg-[#0c1013] py-14 sm:py-20"><div className="container grid gap-10 xl:grid-cols-[minmax(0,1fr)_300px]">
      <div><div className="mb-8 text-xs text-white/45"><Link href="/articles" className="transition hover:text-steel-orange">Инженерный журнал «Сталь Продукт»</Link><span className="mx-2 text-steel-orange">/</span>{article.category}</div>{article.series ? <p className="mb-4 text-[10px] font-bold uppercase tracking-[.16em] text-steel-orange">{article.series}</p> : null}<p className="max-w-3xl text-lg leading-relaxed text-white/78">{article.lead}</p><div className="mt-10 space-y-10">{article.sections.map((section, index) => <section key={section.title} className="border-l border-white/12 pl-5 sm:pl-7"><p className="font-mono text-sm font-bold text-steel-orange">{String(index + 1).padStart(2, "0")}</p><h2 className="mt-3 text-2xl font-semibold leading-tight sm:text-3xl">{section.title}</h2>{section.paragraphs?.map((paragraph) => <p key={paragraph} className="mt-5 text-base leading-8 text-white/66">{paragraph}</p>)}{section.newsItems?.length ? <div className="mt-7 grid gap-5">{section.newsItems.map((item, itemIndex) => <article key={item.title} className="border border-white/12 bg-[#111519] p-5 sm:p-7"><div className="flex items-start gap-4"><span className="font-mono text-sm font-bold text-steel-orange">{String(itemIndex + 1).padStart(2, "0")}</span><h3 className="text-lg font-semibold leading-snug text-white sm:text-xl">{item.title}</h3></div><dl className="mt-6 grid gap-5"><div><dt className="text-[10px] font-bold uppercase tracking-[.13em] text-steel-orange">Краткое описание</dt><dd className="mt-2 text-sm leading-7 text-white/66">{item.summary}</dd></div><div><dt className="text-[10px] font-bold uppercase tracking-[.13em] text-steel-orange">Почему это важно</dt><dd className="mt-2 text-sm leading-7 text-white/66">{item.importance}</dd></div><div><dt className="text-[10px] font-bold uppercase tracking-[.13em] text-steel-orange">Возможные последствия</dt><dd className="mt-2 text-sm leading-7 text-white/66">{item.consequences}</dd></div></dl></article>)}</div> : null}</section>)}</div>{article.sources?.length ? <section className="mt-12 border border-white/12 bg-[#111519] p-6 sm:p-8"><p className="eyebrow">Источники и редакционная проверка</p>{article.editorNote ? <p className="mt-4 max-w-3xl text-sm leading-7 text-white/62">{article.editorNote}</p> : null}<div className="mt-6 grid gap-3">{article.sources.map((source, index) => <a key={source.url} href={source.url} target="_blank" rel="noreferrer" className="group flex items-start gap-4 border-t border-white/10 pt-4 transition hover:border-steel-orange/60"><span className="font-mono text-sm font-bold text-steel-orange">{String(index + 1).padStart(2, "0")}</span><span><strong className="block text-sm font-semibold text-white group-hover:text-steel-orange">{source.name}</strong><span className="mt-1 block text-xs leading-5 text-white/52">{source.title}{source.language ? ` · ${source.language}` : ""}</span></span><span className="ml-auto text-steel-orange">↗</span></a>)}</div></section> : null}</div>
