...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    context = requirePdApiContext(request, undefined, { allowPasswordChange: true, environment: productionAccessEnvironment() });
    ...
    const response = NextResponse.json({ ok: true, next: "/internal/production-calculator" }, { headers: pdPrivateHeaders() });
    setPdSessionCookies(response, session, true);
    return response;
  } catch (error) {
