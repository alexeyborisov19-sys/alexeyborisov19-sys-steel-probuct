...
export async function GET() {
  try {
    ...
    const options = preAuthCookieOptions(600, true);
  ...
  try {
    ...
    const response = NextResponse.json({
      ...
      next: result.mustChangePassword
        ? "/internal/production-access/change-password"
        : "/internal/production-calculator",
    }, { headers: pdPrivateHeaders() });
    setPdSessionCookies(response, result.session, true);
    const preAuthCookie = preAuthCookieOptions(0, true);
