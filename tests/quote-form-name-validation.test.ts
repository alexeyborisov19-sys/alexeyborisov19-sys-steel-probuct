...
test("quote form validates the required name before the network request", async () => {
  ...
  assert.doesNotMatch(quoteForm, /focusFormField\(form, "phone"\)/);
  assert.doesNotMatch(quoteForm, /focusFormField\(form, "email"\)/);
  assert.doesNotMatch(quoteForm, /focusFormField\(form, "personalDataConsent"\)/);
