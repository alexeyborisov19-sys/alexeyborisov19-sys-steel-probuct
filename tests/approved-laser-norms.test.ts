...
test('approved equality boundaries pass: diameter = thickness and ligament = 1 mm',()=>{
 assert.ok(check(2,1).every(r=>r.severity==='pass'));
 assert.ok(check(4,1,4).every(r=>r.severity==='pass'));
});
test('a hole or ligament below its approved minimum blocks, larger values pass',()=>{
 assert.equal(check(1.99,1).find(r=>r.code==='feature-rules')?.severity,'error');
 assert.equal(check(2,.99).find(r=>r.code==='feature-rules')?.severity,'error');
