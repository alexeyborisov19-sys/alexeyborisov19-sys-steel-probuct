...
export function SolutionDetailPage({ solution }: { solution: SolutionDetail }) {
  ...
  return <><JsonLd data={[serviceSchema({ name: solution.title, description: solution.description, path: `/solutions/${solution.slug}`, serviceType: seo.serviceType ?? solution.eyebrow }), faqSchema(faqItems)]} /><PageLayout path={`/solutions/${solution.slug}`} eyebrow={solution.eyebrow} title={solution.title} description={solution.description} image={solution.image} imageAlt={`${solution.title} — изделия из листового металла «Сталь Продукт»`} imageBrightness={brightenIndustry}>
