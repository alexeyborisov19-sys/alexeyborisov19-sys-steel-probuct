...
export function CompanyVideo() {
  ...
  return <>
    <JsonLd data={videoSchema({
      name: "Сталь Продукт — производство и команда",
      description: "Фильм о команде, инженерной экспертизе и производстве продукции под брендом «Сталь Продукт».",
    ...
    <section ref={sectionRef} id="company-video" className="bg-[#0c1013] py-14 sm:py-16">
      <div className="container grid gap-8 lg:grid-cols-[minmax(0,.64fr)_minmax(0,1fr)] lg:items-center">
        <div className="max-w-xl">
          <p className="eyebrow">Фильм о производстве</p>
          <h2 className="mt-3 text-3xl font-semibold leading-tight sm:text-4xl">Познакомьтесь со «Сталь Продукт»</h2>
          <p className="mt-5 text-sm leading-relaxed text-white/62 sm:text-base">Видео о команде, инженерной экспертизе и реальном производстве. Показываем, как команда решает задачи клиентов — от идеи до готового изделия.</p>
        ...
        <button type="button" onClick={(event) => openVideo(event.currentTarget)} className="production-video-frame group text-left" aria-label="Смотреть фильм о производстве Сталь Продукт">
          ...
          <span className="pointer-events-none absolute inset-x-0 bottom-0 flex items-end justify-between bg-gradient-to-t from-black/85 via-black/15 to-transparent px-5 pb-5 pt-16">
            <span className="text-xs font-bold uppercase tracking-[.16em] text-white/75">«Сталь Продукт» — производство и команда</span>
    ...
    {isOpen ? <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Фильм о производстве Сталь Продукт">
