...
export function CommercialProductLandingPage({ landing }: { landing: CommercialProductLanding }) {
  return <>
    ...
    >
      ...
      <section className="border-t border-white/10 bg-[#17191a] py-10"><div className="container flex flex-col justify-between gap-6 md:flex-row md:items-center"><div><p className="eyebrow">Расчёт под задачу</p><h2 className="mt-2 text-2xl font-semibold uppercase">Пришлите чертёж или описание</h2><p className="mt-3 text-sm text-white/58">Проверим исходные данные, подтвердим технологию и подготовим предметное предложение.</p></div><Link href="/contacts#contact-form" className="clip-corner whitespace-nowrap bg-steel-orange-deep px-8 py-4 text-sm font-bold uppercase">Получить расчёт&nbsp; →</Link></div></section>
    </PageLayout>
  </>;
}
