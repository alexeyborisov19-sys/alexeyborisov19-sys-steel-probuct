...
export function InnerHero({ eyebrow, title, titleAccent, description, image, imageAlt, imageBrightness = false }: InnerHeroProps) {
  return <section className="relative isolate min-h-[620px] overflow-hidden pt-[88px]">
    ...
    {image ? <img src={image} alt={imageAlt ?? ""} aria-hidden={imageAlt ? undefined : "true"} width={1280} height={720} loading="eager" fetchPriority="high" decoding="async" className="absolute inset-0 h-full w-full object-cover" /> : null}
