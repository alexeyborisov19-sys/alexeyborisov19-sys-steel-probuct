...
Проект проверен Gitleaks `8.29.0`. Бинарный файл устанавливается из
официального GitHub Release с проверкой SHA-256 и не добавляется в Git. Можно
указать проверенный бинарный файл через `GITLEAKS_BIN` либо установить
`gitleaks` в `PATH`.
...
Команда проверяет полную Git-историю и текущее рабочее дерево с обязательным
редактированием совпадений. В `.gitleaksignore` допускаются только точные
fingerprint исторических false positive. Широкие исключения по каталогам
тестов или правилам запрещены. Тестовые ключи формируются во время запуска и
никогда не используются в production.
