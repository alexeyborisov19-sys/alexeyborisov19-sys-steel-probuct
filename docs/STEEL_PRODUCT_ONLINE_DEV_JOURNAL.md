## Актуальный контракт доставки — 21.09.2026

Владелец подтвердил рабочий компьютер и запуск отдельным приложением. На сайте только клиентская версия. Автоматическое обновление цен обязательно в обеих версиях. Дизайн и содержание проходят реальную браузерную проверку. PR #163 (0f9e76e) остаётся draft и НЕ подлежит merge на сайт. Прежние NEXT ACTION о merge #163 ниже отменены. Первый администратор создаётся локально, серверный bootstrap не запускается.

NEXT ACTION: собрать отдельную локальную поставку, проверить вход/расчёт/автоцены; выделить клиентские изменения в чистую ветку от main и публиковать только её. Найденный дефект Node 22 в backup bootstrap исправлен через SQLite VACUUM INTO, ожидает проверки нового SHA.

## Уточнение владельца: производственное приложение НЕ на сайте

Пользователь указал, что производственный калькулятор должен работать отдельно от сайта. Merge/deploy производственных маршрутов на основной сайт остановлен до изменения архитектуры доставки. PR #163 остаётся рабочей веткой; новые изменения ещё не публиковались. Первый production-администратор ещё не создан. Подтверждённый таймер прайсов продолжает существующую разрешённую работу. Уточняется место запуска отдельного браузерного приложения: рабочий компьютер или отдельный сервер сотрудников.

## Live checkpoint — продолжение до публикации, 21.09.2026

Уточнение владельца: нужен первый администратор. Live-проверка показала, что существующий PD-вход выключен. Добавлен отдельный opt-in `STEEL_PRODUCT_PRODUCTION_APP_ENABLED` и маршруты production-access, использующие проверенные auth/session/password механизмы с отдельным server-side environment. Сохранённый PD_ADMIN_ENABLED не изменяется; личные данные/юридический кабинет не включаются. Создание администратора доставляет временный пароль только RSA-OAEP ciphertext, требует смены при первом входе и отказывается перезаписывать существующую учётную запись. Проверены реальный вход, принудительная смена пароля, CSRF отказ, logout и сохранение 404 для PD.

Автоцены: run 35646144664 успешно включил штатный таймер и выполнил реальный `--commit`: 25 строк, persisted=true, служба exit 0. Это подтверждённая работа production-обновления.

DONE перед выпуском: самостоятельное приложение прошло авторизованный браузерный сценарий с реальным серверным расчётом на синтетических ставках, сохранением и чтением отчёта, скрытием устаревшего аудита и пятью размерами экрана. Публичный сценарий прошёл шесть размеров экрана, zoom и защиту от задержанной старой цены. 1234 теста, lint, typecheck, build и SEO (89 URL, 18 redirects, 7 gone) пройдены локально.

Read-only production preflight run 35645803513 подтвердил активный старый release 179d00e, наличие каталога отчётов и отсутствие настроенной Yandex-модели/поиска. Публикация интерфейса не выдаётся за активацию модели. NEXT ACTION: CI нового коммита, merge #163, штатный deploy, проверка живых маршрутов и таймера.

Владелец явно поручил продолжить построение до создания и публикации. Разрешение распространяется на commit/push, завершение PR #163, штатный deploy и проверку опубликованного приложения. Оплата/checkout и новые платные сервисы не включаются.

База подтверждена через GitHub: #163 открыт draft, HEAD 5556806, base 179d00e; локальные изменения перечислены предыдущим checkpoint. NEXT ACTION: авторизованный браузерный сценарий на синтетической локальной базе, исправление обнаруженных дефектов, итоговые gates, CI нового SHA, merge/deploy и live-проверка.

## Дополнение 21.09.2026: самостоятельное приложение и найденный ИИ-модуль

Выделены ProductionApplication и shared useCadProject; добавлен просмотр сохранённого quoteControl с проверкой ID текущего расчёта. Найден существующий модуль из #157, он уже в базе и повторно не переносился. В старой копии от 17.09 два файла с суффиксом «2» идентичны оригиналам и сохранены. Подробности: docs/cad-calculator-v2/legacy-ai-inventory.md.

Рабочее дерево: тесты 1234/1234, lint/build и публичный браузерный сценарий пройдены. Авторизованная приёмка нового внутреннего интерфейса, реальные ставки/модель и производственный таймер ещё не подтверждены. Изменения не опубликованы; работа по полному заданию продолжается.
...
## Live checkpoint — 21.09.2026, CAD v2 и автоматические цены

- Рабочая копия: `/Users/alex/.codex/steelprodukt-cad-20260921`; исходная папка задачи отсутствует. Старые копии не изменялись.
- База: PR #163, `feat/cad-tier-split-final-20260921`, HEAD `55568061245aa9cf5c1fca97375917a2e7815eb4`; PR открыт, draft, не слит. `main`: `179d00e615b6e5639429677c07a75c50944a91fd`.
- GitHub Actions run `35640004861`: success именно для исходного HEAD. Новые изменения требуют новых проверок.
- Старый checkpoint `e59cef6` отсутствует в локальной истории этой ветки. Проверены 8 коммитов PR над main: разделение входов и навигации, раскрытие операций, тесты. Старый NEXT ACTION заменён текущим заданием владельца: продолжить два калькулятора и автоматические цены.
- DONE: найден действующий Atlantik PDF adapter, CLI, таймер, приватная база и тесты. Это не доказательство активного таймера production.
- IN PROGRESS: устранение гонки клиентских расчётов, упрощение компоновки, проверка свежести и выбора прайса, наблюдаемость источников.
- NEXT ACTION: проверить изменения тестами и браузером; записать точные ограничения в `docs/cad-calculator-v2/acceptance.md`.
- Новые платные сервисы, checkout и изменения калькулятора металлокассет не входят в работу. Публикация не выполнена.
